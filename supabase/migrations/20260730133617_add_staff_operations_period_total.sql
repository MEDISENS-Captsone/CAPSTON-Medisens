-- Add exact selected-period turnaround rollups to the Doctor-only G4B RPC.
--
...
-- calculated directly from valid workflow intervals. Period totals are never
-- calculated by combining staff-level or bucket-level averages or medians.
--
...
begin
  ...
  turnaround_period_metrics as (
    select *
    from (
      values
        ('lab_turnaround_minutes'::text, 'laboratory'::text),
        ('prescription_turnaround_minutes'::text, 'pharmacist'::text)
    ) as metrics(metric_key, role_key)
  ),
  turnaround_period_rollups as (
    select
      te.metric_key,
      te.role_key,
      pg_catalog.round(pg_catalog.avg(te.duration_minutes), 2)
        as duration_minutes_avg,
      pg_catalog.round(
        pg_catalog.percentile_cont(0.5)
          within group (order by te.duration_minutes)::numeric,
        2
      ) as duration_minutes_median
    from turnaround_events as te
    group by te.metric_key, te.role_key
  ),
  turnaround_period_counts as (
    select
      sr.metric_key,
      sr.role_key,
      pg_catalog.sum(sr.attributed_count)::bigint as attributed_count,
      pg_catalog.sum(sr.unattributed_count)::bigint as unattributed_count
    from staff_rows as sr
    where sr.metric_group = 'turnaround'
    group by sr.metric_key, sr.role_key
  ),
  scoped_rows as (
    select
    ...
    select
      ...
      p_from,
      null::bigint,
      tpr.duration_minutes_avg,
      tpr.duration_minutes_median,
      coalesce(tpc.attributed_count, 0::bigint),
      coalesce(tpc.unattributed_count, 0::bigint),
      'approximate'::text,
      'workflow_events_role_total'::text
    from turnaround_period_metrics as tpm
    left join turnaround_period_rollups as tpr
      on tpr.metric_key = tpm.metric_key
      and tpr.role_key = tpm.role_key
    left join turnaround_period_counts as tpc
      on tpc.metric_key = tpm.metric_key
      and tpc.role_key = tpm.role_key
  )
  select
...
comment on function public.analytics_staff_operations_g4b(date,date,text) is
  'Doctor-only aggregate Staff Operations metrics with staff, bucket-level role_total, and exact selected-period turnaround scopes. Period averages and medians are calculated directly from all valid workflow intervals. Date-only starts remain approximate Asia/Manila local-midnight values.';
